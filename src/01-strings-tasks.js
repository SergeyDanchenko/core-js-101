...
function extractNameFromTemplate(value) {
  ...
  const result = arr[1].slice(0, arr[1].length - 1);
  return result;
}
...
function encodeToRot13(str) {
  ...
  const secondAlphabet = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm !?';
  for (let i = 0; i < str.length; i += 1) {
    result += secondAlphabet[firstAlphabet.indexOf(str[i])];
  }
  return result;
}
